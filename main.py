...
class App(tk.Tk):
    def __init__(self):
        ...
        try:
            ...
            self.btn_start = tk.Button(entry_block_start_date, text="Выбор даты начала расчета",
                                       command=lambda m="start": self.pick_date(m))
            ...
            self.btn_end = tk.Button(entry_block_end_date, text="Выбор даты окончания расчета",
                                     command=lambda m="end": self.pick_date(m))
            ...
            self.msg_end_date.grid(row=1, column=0, sticky='ns', columnspan=3)

            # блок выбора применения ставки

            entry_option_bet = tk.Frame(fr_buttons)

            self.bet_option_var = tk.IntVar()

            self.bet_option_var.set(0)
            self.option_0 = tk.Radiobutton(entry_option_bet, text='по периодам действия', variable=self.bet_option_var,
                                           value=0)
            self.option_1 = tk.Radiobutton(entry_option_bet, text='на конец периода начисления неустойки',
                                           variable=self.bet_option_var,
                                           value=1)
            self.option_2 = tk.Radiobutton(entry_option_bet, text='на сегодня', variable=self.bet_option_var,
                                           value=2)
            self.option_3 = tk.Radiobutton(entry_option_bet, text='на выбранную дату', variable=self.bet_option_var,
                                           value=3)

            self.btn_choose = tk.Button(entry_option_bet, text="Выбор даты",
                                        command=lambda m="choose": self.pick_date(m))
            self.text_choose_date = tk.Label(entry_option_bet, font=("Calibri", 12), text="ДД.ММ.ГГГГ")
            self.msg_choose_date = tk.Label(entry_option_bet, font=("Calibri", 12), foreground='#FF2D00')

            self.option_0.grid(row=0, column=0, columnspan=3, sticky='ns')
            self.option_1.grid(row=1, column=0, columnspan=3, sticky='ns')
            self.option_2.grid(row=2, column=0, columnspan=3, sticky='ns')
            self.option_3.grid(row=3, column=0, sticky='ns')
            self.btn_choose.grid(row=3, column=1, sticky='ns', padx=10)
            self.text_choose_date.grid(row=3, column=2, sticky='ns', columnspan=2)
            self.msg_choose_date.grid(row=4, column=0, columnspan=3, sticky='ns')
            ...
            balloon_visualization.bind(self.check_box_visualization, text_for_balloon_visualization)
            ...
            entry_option_bet.grid(row=7, column=0, columnspan=2, sticky='ew', pady=5)
    ...
    def collect_data_for_selenium(self):
        try:
            ...
            start_date = datetime.strptime(self.text_start_date.cget("text"), "%d.%m.%Y").date()
            end_date = datetime.strptime(self.text_end_date.cget("text"), "%d.%m.%Y").date()

            if (end_date > start_date):
                self.msg_end_date.config(text="")
            else:
                self.msg_end_date.config(text="Дата окончания периода не может\nбыть раньше даты начала периода")
                return
            ...
            dict_with_data['bet_option_var'] = self.bet_option_var.get()

            choose_date = ""
            choose_period = self.text_choose_date.cget("text").split('.')

            if self.bet_option_var.get() == 3:

                if choose_period[0].isnumeric():
                    self.msg_choose_date.config(text="")
                else:
                    self.msg_choose_date.config(text="Необходимо выбрать дату ставки!")
                    return
            else:
                self.msg_choose_date.config(text="")

            choose_date = f"{choose_period[0]}{choose_period[1]}{choose_period[2]}"
            dict_with_data['choose_date'] = choose_date
    ...
    def grab_date(self, button_press):
        ...
        elif button_press == "choose":
            self.text_choose_date.config(text=cal.get_date())
        date_window.destroy()
